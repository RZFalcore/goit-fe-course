...
const getInactiveUsers = users => {
    let arr = users.filter(user => user.isActive === false)
    return arr;
  };
...
const getTotalBalance = users => {
  let total = users.reduce((count, user) => count += user.balance,0);
  return total;
};
...
const getUsersByFriend = (users, name) => {
  let arr = users.filter(user => user.friends.includes(name))
  return arr;
};
